...
exports.handler = async (event) => {
    ...
    if (entries.length) {
        ...
        const lambdaResults = await Promise.all(checkPhotosParams.map(x => lambdaClient.invoke(x)));

        const results = lambdaResults.map(({ Payload }) => JSON.parse(Buffer.from(Payload)));
